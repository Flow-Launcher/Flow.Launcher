...
public partial class LocalizeSourceGenerator : ISourceGenerator
{
    ...
    private readonly Regex _languagesXamlRegex = RegexEndsWithLanguagesXaml();
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        ...
        context.Compilation.SyntaxTrees
            ...
            .ForEach(
                v =>
                {
                    ...
                    if (split[0] is not ClassName) return;
        ...
        var unusedLocalizationKeys = localizedStrings.Keys.Except(allLanguageKeys).ToHashSet();
    ...
    private static Dictionary<string, LocalizableString> LoadLocalizedStrings(IEnumerable<AdditionalText> files)
    {
    ...
    private static bool CommentIncludesDocumentationMarkup(XComment comment) {
        return comment.Value.Contains("<summary>", StringComparison.OrdinalIgnoreCase) ||
               comment.Value.Contains("<param ", StringComparison.OrdinalIgnoreCase);
    }

    private string GenerateSourceCode(
        Dictionary<string, LocalizableString> localizedStrings,
        GeneratorExecutionContext context,
        HashSet<string> unusedLocalizationKeys
    )
    {
        var ns = context.Compilation.AssemblyName;

        var sb = new StringBuilder();
        if (ns is CoreNamespace1 or CoreNamespace2)
        {
    ...
    private static void GenerateDocCommentForMethod(StringBuilder sb, LocalizableString localizableString)
    {
        ...
        if (localizableString.Summary is not null)
        {
    ...
    private static void GenerateMethod(StringBuilder sb, LocalizableString localizableString, string? contextPropertyName)
    {
        ...
        var callArray = callArgs.Count switch
        {
            0 => "",
            1 => callArgs[0],
            _ => $"new object?[] {{ {string.Join(", ", callArgs)} }}"
        };
    ...
    private record Modifiers(bool Static = false, bool Private = false, bool Protected = false);

    [GeneratedRegex(@"\\Languages\\[^\\]+\.xaml$", RegexOptions.IgnoreCase)]
    private static partial Regex RegexEndsWithLanguagesXaml();
}

public record LocalizableString(string Key, string Value, string? Summary, IEnumerable<LocalizableStringParam> Params);
public record LocalizableStringParam(int Index, string Name, string Type);
