...
public class ContextAvailabilityAnalyzerCodeFixProvider : CodeFixProvider
{
    ...
    private Document FixContextNotDeclared(CodeFixContext context, SyntaxNode root, TextSpan diagnosticSpan)
    {
        var classDeclaration = GetDeclarationSyntax<ClassDeclarationSyntax>(root, diagnosticSpan);
        if (classDeclaration is not { BaseList: not null }) return context.Document;
    ...
    private static Document FixContextIsTooRestricted(CodeFixContext context, SyntaxNode root, TextSpan diagnosticSpan)
    {
        ...
        {
            newModifiers = newModifiers.Add(
                modifier.Kind() switch
                {
                    SyntaxKind.PrivateKeyword or SyntaxKind.ProtectedKeyword => SyntaxFactory.Token(
